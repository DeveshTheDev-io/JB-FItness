...
const chatEndpointStart = code.indexOf('app.post("/api/ai/chat"');
const dietTrackerStart = code.indexOf('app.post("/api/ai/diet-tracker"');

if (chatEndpointStart > -1 && dietTrackerStart > -1) {
  const newChatEndpoint = `app.post("/api/ai/chat", async (req, res) => {
    try {
  ...
  const before = code.substring(0, chatEndpointStart);
  const after = code.substring(dietTrackerStart);
  fs.writeFileSync('server.ts', before + newChatEndpoint + after);
}
